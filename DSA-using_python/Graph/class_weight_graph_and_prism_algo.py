...
class weight_graph:
    ...
    def minimumweight_notvisited_vertex(self,visited,weight):
        min_weight=min([weight[i] for i in range(len(weight)) if visited[i]==False])
        return weight.index(min_weight)

    def mst_using_prism_algo(self):
        visited=[False for i in range(self.vertex)]
        weight=[sys.maxsize for i in range(self.vertex)]
        parent=[-1 for i in range(self.vertex)]
        weight[0]=0
        for i in range(self.vertex-1):
            min_vertex=self.minimumweight_notvisited_vertex(visited,weight)
